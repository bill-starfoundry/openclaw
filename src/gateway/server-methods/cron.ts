...
async function assertValidCronUpdatePatch(params: {
  ...
  const touchesWebhookCompatibility = "delivery" in params.patch || "sessionTarget" in params.patch;
  if (
    touchesWebhookCompatibility &&
    nextJob.sessionTarget === "main" &&
    nextJob.delivery?.mode === "webhook"
  ) {
    throw new Error("main-session cron jobs do not support webhook delivery");
  }
