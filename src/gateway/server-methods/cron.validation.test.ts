...
function createCronContext(currentJobs?: CronJob | CronJob[]) {
  ...
  return {
    cron: {
      add: vi.fn(async (input: Partial<CronJob>) => createCronJob({ ...input, id: "cron-1" })),
      update,
...
describe("cron method validation", () => {
  ...
  it("rejects webhook delivery for main-session adds and updates", async () => {
    ...
    expectResponseError(add.respond, { messageIncludes: "do not support webhook delivery" });
    ...
    expectResponseError(update.respond, { messageIncludes: "do not support webhook delivery" });
  });
  ...
  it("revalidates delivery against the locked cron.update snapshot", async () => {
    ...
    expectResponseError(respond, { messageIncludes: "do not support webhook delivery" });
  });
